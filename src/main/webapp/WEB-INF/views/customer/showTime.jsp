        ...
        function updateMovieSchedule(idMovie) {
            $.get("http://localhost:8080/RapChieuPhim_SpringMVC/api/lichchieu", {
                ...
                if (response.ngaychieuOptions && response.ngaychieuOptions.length > 0) {
                        ...
                        response.listmovietime.forEach(function(time) {
                            ...
                            if (date == formattedDate && time.idMovie.idMovie == idMovie) {
                              ...
                              /*   if (time.status == 1) {
                                ...
                                if (time.status == 1) {
                                    movieItem += '<a href="<c:url value="/pickchair?idMovieTimeRoom=' + time.idMovieTimeRoom + '&idRoom=${time.getIdRoom()}&idMovie=${time.idMovie.getIdMovie()}"/>" class="box-time-hour text-decoration-none text-white"><div>' + time.beginTime.substring(0, 5) + '</div></a>';
                                } else {
                                    movieItem += '<a href="" class="box-time-hour-pass text-decoration-none text-white">' + time.beginTime.substring(0, 5) + '</a>';
                                }
                    ...
                    movieSchedule.html('<p>Không có ngày chiếu nào được hiển thị.</p>');
                }
            });
        }
