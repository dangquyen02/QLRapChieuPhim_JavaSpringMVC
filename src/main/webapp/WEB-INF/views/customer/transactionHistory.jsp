...
<body class="bg-dark">
    ...
    <div class="content">
        <div class="row w-100">
            <div class="col-3"></div>
            <div class="col-6 text-center mt-5 bg-white border rounded " style="height: 500px;">
                <h3 class="mt-4" style="color: #246d69;">Lịch sử giao dịch Online</h3>
                <table class="table mt-4 table-bordered table-striped">
                    ...
                    <c:forEach items="${listTr }" var="transaction">
                        <tr>
                            <td>${transaction.getTicketCode()}</td>
                            <td><fmt:formatDate value="${transaction.getBookingTime()}" pattern="dd-MM-yyyy" /></td>
                            <td><fmt:formatNumber value="${transaction.getTotalAmount()}" type="currency" currencySymbol="đ" /></td>
                            <td>
                            	<c:choose>
									<c:when test="${transaction.getStatus()==0}">Đã hủy vé</c:when>
									<c:when test="${transaction.getStatus() == 1 }">Đặt vé thành công</c:when>
									<c:when test="${transaction.getStatus() == 2 }">Đặt vé không thành công</c:when>
								</c:choose>
                            </td>
                            <td>
                                <a data-bs-toggle="modal" data-bs-target="#exampleModal${transaction.getTicketCode()}" href="#" class="btn" style="background-color:#28817d; font-size: 8px;">
                                    <i style="font-size: 16px; color: #fff;" class="fa fa-list"></i>
                                </a>
                            </td>
                        </tr>
                        </c:forEach>
                ...
                <!-- Modal -->
                <c:forEach items="${listTr }" var="transaction">
                <div class="modal fade" id="exampleModal${transaction.getTicketCode()}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog" style="max-width: 35%;">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title">CHI TIẾT GIAO DỊCH</h5>
                            ...
                            <div class="modal-body">
                                <table class="table text-left">
                                    <tbody>
                                        ...
                                        <tr>
                                            <th scope="row">Thời điểm:</th>
                                            <td style="text-align: left;"><fmt:formatDate value="${transaction.getBookingTime()}" pattern="dd-MM-yyyy" /></td>
                                        </tr>
                                        <tr>
                                            <th scope="row">Khách hàng:</th>
                                            <td style="text-align: left;">${transaction.getFullName()}</td>
                                        ...
                                        <tr>
                                            <th scope="row">Suất chiếu:</th>
                                            <td style="text-align: left;">${transaction.getBeginTime()}</td>
                                        </tr>
                                        <tr>
                                            <th scope="row">Phòng chiếu:</th>
                                            <td style="text-align: left;">${transaction.getRoomCode()}</td>
                                        ...
                                        <tr>
                                            ...
                                            <td style="text-align: left;">
                                            	<c:forEach items="${getdetail}" var="detail" varStatus="loop">
											        <c:if test="${detail.getTicketCode() eq transaction.getTicketCode()}">
											            ${detail.getChairCode()}
                                        ...
                                        <tr>
                                            ...
                                            <td style="text-align: left;">
                                            	<c:choose>
													<c:when test="${transaction.getRefund() == true}">Đã thanh toán</c:when>
													<c:when test="${transaction.getRefund() == false }">Chưa thanh toán</c:when>
												</c:choose>
											</td>
                                        </tr>
                                    </tbody>
                            ...
                            <div class="modal-footer">
                                <p><strong>Tổng tiền:</strong> <fmt:formatNumber value="${transaction.getTotalAmount()}" type="currency" currencySymbol="đ" /></p>
                            </div>
                        </div>
                    </div>
                </div>
