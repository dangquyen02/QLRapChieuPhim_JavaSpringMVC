...
<body>
	<div class="content-register pb-4">
		<div class="row">
			...
			<div class="col-md-4 frm-register mt-5">
				...
				<c:if test="${not empty message}">
					<div
						style="text-align: center; color: green; font-size: 16px; font-weight: bold;"
					...
					<div class="frm-register-content row justify-content-center">
						<div class="col-md-10 mb-4">
							...
							<c:if test="${not empty message_fullName}">
								<div style="padding: 0px; color: red; margin: 0;"
									class="alert alert-${alert}">${message_fullName}</div>
							</c:if>
						</div>
						...
						<div class="col-md-10 mb-4">
							...
							<c:if test="${not empty message_dateOfBirth}">
								<div style="padding: 0px; color: red; margin: 0;"
									class="alert alert-${alert}">${message_dateOfBirth}</div>
							</c:if>
						</div>
						...
						<div class="col-md-10 mb-4">
							...
							<c:if test="${not empty mess_phone}">
								<div style="padding: 0px; color: red; margin: 0;"
									class="alert alert-${alert}">${mess_phone}</div>
							</c:if>
						</div>
						...
						<div class="col-md-10 mb-4">
							...
							<c:if test="${not empty message_address}">
								<div style="padding: 0px; color: red; margin: 0;"
									class="alert alert-${alert}">${message_address}</div>
							</c:if>
						...
						<div class="col-md-10 mb-4">
							...
							<c:if test="${not empty message_email}">
								<div style="padding: 0px; color: red; margin: 0;"
									class="alert alert-${alert}">${message_email}</div>
							</c:if>
						...
						<div class="col-md-10 mb-4">
							...
							<c:if test="${not empty message_password}">
								<div style="padding: 0px; color: red; margin: 0;"
									class="alert alert-${alert}">${message_password}</div>
							</c:if>
						</div>
						...
						<div class="col-md-10 mb-4">
							...
							<c:if test="${not empty message_repeat_password}">
								<div style="padding: 0px; color: red; margin: 0;"
									class="alert alert-${alert}">${message_repeat_password}</div>
							</c:if>
						</div>

						<div class="frm-register-footer col-md-12 text-center">
							<p>
								Bạn đã có tài khoản? <a href="dangnhap"
									class="text-primary text-decoration-none">Đăng nhập</a>
							</p>
							<button style="" type="submit" class="btn mb-2" name="click_true">Đăng
								ký</button>
						</div>
					</div>
				</form>
