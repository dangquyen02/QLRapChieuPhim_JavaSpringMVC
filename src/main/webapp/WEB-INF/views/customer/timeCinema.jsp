...
<body class="bg-dark">
<div class="lichchieu-content border-0 pb-5">
	<div class="container">
		<div class="row">
			<div class="col-sm-12">
...
movieItem += '<div class="movie-list-infor mt-2">';
                        ...
                        response.listmovietime.forEach(function(time) {
                          ...
                          var formattedDate = day + '/' + month + '/' + year;
                            if (time.idMovie.idMovie == movie.idMovie.idMovie && formattedDate == selectedDate) {
                                if (time.status == 1) {
                                	movieItem += '<a href="<c:url value="/pickchair?idMovieTimeRoom=' + time.idMovieTimeRoom + '&idRoom='+time.getIdRoom()+'&idMovie='+time.getIdMovie().getIdMovie()"/>"  class="text-decoration-none"><span class="time item">' + time.beginTime.substring(0, 5) + '</span></a>';
                                } else {
                    ...
                    movieSchedule.html('<p>Không có phim được hiển thị.</p>');
                }
            }
        );
    }
