...
<body>
	...
	<script>
	    ...
	    var weekRevenueData = {
	        labels: [<c:forEach items="${lst_date}" var="date" varStatus="loop">
	                    /* '<c:out value="${date}"/>' */
    ...
    '${formattedDate}'
	        ...
	        datasets: [{
	            label: 'Doanh thu',
	            data:  [<c:forEach items="${lst_dailyrevenue}" var="dr" varStatus="loop">
                <c:out value="${dr.total}"/><c:if test="${!loop.last}">,</c:if>
                </c:forEach>],
