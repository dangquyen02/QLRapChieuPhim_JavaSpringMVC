					...
					<div class="box box-primary">
						<div class="box-body">
									...
									<div class="form-group col-sm-3">
										...
										<select class="form-control" id="status" name="status">
											<option value=""
												...
												thái--</option>
											<option value="1"
												<c:if test="${searchStatus == '1'}">selected</c:if>>Thành công</option>
											<option value="0"
												<c:if test="${searchStatus == '0'}">selected</c:if>>Đã hủy</option>
											<option value="2"
												<c:if test="${searchStatus == '2'}">selected</c:if>>Không thành công</option>
										</select>
									</div>
									<!-- <div class="form-group col-sm-3">
												...
												<c:forEach items="${listticket}" var="ticket">
													...
													<c:if test="${ticket.getId_Ticket() != id}">
														...
														<tr>
															...
															<%-- <td><c:forEach items="${uniqueCombo }" var="uniqueCombo">
																${uniqueCombo.getComboName()}
															</c:forEach></td> --%>
															...
															<%-- <td><c:choose>
																	<c:when test="${ticket.getRefund() == true}">Đã thanh toán</c:when>
																	<c:when test="${ticket.getRefund() == false }">Chưa thanh toán</c:when>
																	<c:when test="${ticket.getStatus() == 0 }">Đã hoàn tiền</c:when>
																</c:choose></td> --%>
															...
															<tbody>
																...
																<tr>
																	...
																	<%-- <c:set var="temp" value="false"/>
																		...
																		<%-- <c:set var="temp" value="false"/>
																		<c:forEach items="${uniqueChair}" var="chair">
																			
																		    <c:if test="${chair.getId_Ticket() eq ticket.getId_Ticket()}">
																...
																<tr>
																	...
																	<td>
																		<c:set var="temp" value="false"/>
																		<c:forEach items="${comboticket }" var="comboticket">
																			<c:if test="${comboticket.getIdTicket() eq ticket.getId_Ticket() }">
																				<c:forEach items="${combo }" var="combo">
																					<c:if test="${comboticket.getIdCombo() eq combo.getIdcombo() }">
																						<c:if test="${temp}">,</c:if>
																							${combo.getComboname() }
																						<c:set var="temp" value="true"/>
																					</c:if>
							...
							<c:choose>
								...
								<c:otherwise>
									<div class="text-center" role="alert">Không có
										kết quả nào được tìm thấy.</div>
								</c:otherwise>
							</c:choose>
						...
						<div class="box box-primary">
							<div class="box-body">
								<form action='<c:url value = '/admin/ticket'/>' method="get">
									<div class="form-row d-flex align-items-center">
										<div class="form-group col-sm-3">
											...
											<select class="form-control" id="status" name="status">
											    <option value="" <c:if test="${empty searchStatus}">selected</c:if>>--Trạng thái--</option>
											    <option value="1" <c:if test="${searchStatus == '1'}">selected</c:if>>Thành công</option>
											    <option value="0" <c:if test="${searchStatus == '0'}">selected</c:if>>Đã hủy</option>
											    <option value="2" <c:if test="${searchStatus == '2'}">selected</c:if>>Không thành công</option>
											</select>
										</div>
										<!-- <div class="form-group col-sm-3">
                                            <input type="text" name="daterange" class="form-control daterange" value="" placeholder="" />
										...
										<div class="form-group col-sm-9">
											<div class="input-group">
												<input type="text" id="name" value="${key}" name="txtsearch" class="form-control"
													placeholder="Nhập tên khách hàng hoặc phim cần tìm"
													autofocus>
												<div class="input-group-append">
													<button name="but1" class="btn btn-info" type="submit">
								...
								<div>
									<c:if test="${not empty listticket}">
									<p style="margin: 10px 0 10px 0">
										Có <strong>${totalrow }</strong> vé trong tổng số <strong>${totalPages}</strong> trang
									</p>
								</c:if>
									<div class="table-responsive">
										<table class="table table-bordered table-hover table-striped">
													...
													<c:forEach items="${listticket}" var="ticket">
														...
														<c:if test="${ticket.getId_Ticket() != id}">
															...
															<tr>
																...
																<td>
																	<fmt:formatNumber value="${ticket.getTotalAmount()}" type="currency" currencySymbol="đ" />
																</td>
																<td>
																	<c:choose>
																		<c:when test="${ticket.getStatus() == 1 }">Thành công</c:when>
															...
															<table class="table text-left">
																<tbody>
																	...
																	<tr>
																		<th scope="row">Ghế:</th>
																		<td>
																		    <c:forEach items="${getdetail}" var="detail" varStatus="loop">
																		        <c:if test="${detail.getId_Ticket() eq ticket.getId_Ticket()}">
																		            ${detail.getChairCode()}
								...
								<div class="row" style="justify-content: center; margin-top: 24px;">
								    <ul class="pagination">
								        <li class="page-item">
								            <a class="page-link" href="?page=0&amp;status=${param.status}&amp;txtsearch=${param.txtsearch}"><i class="fas fa-angle-double-left"></i></a>
								        </li>
								        <li class="page-item">
								            <c:if test="${currentPage - 1 >= 0}">
								                <a class="page-link" href="?page=${currentPage - 1}&amp;status=${param.status}&amp;txtsearch=${param.txtsearch}"><i class="fas fa-angle-left"></i></a>
								            </c:if>
								        </li>
								        <c:forEach var="pageIndex" begin="1" end="${totalPages}" step="1">
								            <c:set var="pageNumber" value="${pageIndex}" />
								            <li class="page-item ${pageIndex - 1 == currentPage ? 'active' : ''}">
								                <a class="page-link" href="?page=${pageIndex - 1}&amp;status=${param.status}&amp;txtsearch=${param.txtsearch}">${pageNumber}</a>
								            </li>
								        </c:forEach>
								        <li class="page-item">
								            <c:if test="${currentPage + 1 < totalPages}">
								                <a class="page-link" href="?page=${currentPage + 1}&amp;status=${param.status}&amp;txtsearch=${param.txtsearch}"><i class="fas fa-angle-right"></i></a>
								            </c:if>
								        </li>
								        <li class="page-item">
								            <a class="page-link" href="?page=${totalPages - 1}&amp;status=${param.status}&amp;txtsearch=${param.txtsearch}"><i class="fas fa-angle-double-right"></i></a>
								        </li>
								    </ul>
								</div>
							</div>
